...
function verifyToken(req, res, next) {
  ...
  try {
    ...
    return res.status(403).json({ error: 'token לא תקין או פג תוקף' });
  }
}
...
app.use('/api/payers', verifyToken, payersRouter);
...
app.get('/api/students/:studentId/tasks', verifyToken, async (req, res) => {
  try {
...
app.post('/api/tasks/:id/replies', verifyToken, async (req, res) => {
  try {
...
app.post('/api/placements', verifyToken, async (req, res) => {
  try {
...
app.get('/api/placements', verifyToken, async (req, res) => {
  try {
...
app.delete('/api/placements/:id', verifyToken, async (req, res) => {
  try {
...
app.put('/api/placements/:id', verifyToken, async (req, res) => {
  try {
    const placementData = { ...req.body };
...
app.get('/api/scholarships', verifyToken, async (req, res) => {
  try {
...
app.post('/api/scholarships/generate', verifyToken, async (req, res) => {
  try {
...
app.put('/api/scholarships/bulk-pay', verifyToken, async (req, res) => {
  try {
...
app.put('/api/billing/bulk-pay', verifyToken, async (req, res) => {
  try {
    const { ids, isPaid } = req.body;
    if (!ids || !Array.isArray(ids)) {
      return res.status(400).json({ error: 'רשימת מזהים חסרה' });
